...
export async function executeVerifyStep(
  ...
  try {
    deps.metrics?.observeVerifyDuration(verifyResults.reduce((sum, r) => sum + r.durationMs, 0) / 1000)
    deps.metrics?.incVerifyRuns(allVerifyPassed(verifyResults) ? 'pass' : 'fail')
  } catch { /* best-effort */ }
...
function getDefaultTemplate(role: string): string {
  switch (role) {
