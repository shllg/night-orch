...
import { ForgejoForgeAdapter } from './forgejo.js'
...
export function createForgeAdapter(repoConfig: RepoConfig, globalConfig: Config): ForgeAdapter {
  ...
  switch (forgeType) {
    ...
    case 'forgejo': {
      ...
      const apiBaseUrl = repoConfig.apiBaseUrl
      if (!apiBaseUrl) {
        throw new Error('apiBaseUrl is required for Forgejo adapter')
      }
      return new ForgejoForgeAdapter(apiBaseUrl, token)
    }
    default:
