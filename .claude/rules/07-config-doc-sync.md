...
Applies to: `src/config/**`, `src/cli/**`, `src/workers/**`, `src/discovery/**`, `src/loop/**`, `src/forge/**`, `src/environment/**`, `src/runner/**`, `src/mcp/**`, and any file that changes user-visible behavior.

## Documentation Site

The project has a public docs site built with VitePress (`docs/` directory), deployed to GitHub Pages on each release. All user-facing documentation lives in `docs/`:
...
VitePress config: `docs/.vitepress/config.ts`. Build: `pnpm docs:build`.
