...
6. **Environment Setup** — Per-run tokens + host-availability-probed pool ports are allocated, then `check` (validate) and `beforeRun` hooks run (`src/environment/`). Services for stateful verify commands are brought up by each command's `before` hook and torn down by its `after` hook; run-level `afterRun` always runs in a `finally`.
...
### Git (`src/git/`)
All git operations use `execa` (not `simple-git`). `worktree.ts` provides `ensure()` (create or reuse worktree with either fresh-reset or preserve-branch semantics), `remove()`, and `list()`. On reuse, `ensure()` self-heals a dirty worktree: a forceful reset (`reset --hard` + `clean -ffd`), and if cruft still remains (e.g. root-owned files a container dropped that git cannot remove) it discards and recreates the worktree from the branch — committed work survives, only uncommitted cruft is lost. A run is only blocked as dirty if even recreation leaves residue, and the block surfaces the `git status` for diagnosis. `slug.ts` generates and pins branch slugs in the DB so they survive issue title changes. `repo.ts` handles branch operations.
...
One isolation model: a per-run worktree plus **lifecycle hooks**. `manager.ts` allocates per-run substitution tokens (`{issue}`/`{run}`/`{project}` plus one host port per configured pool: `{port}`/`{port:NAME}`) via `prepareEnvironment`, then `runCheckHooks` runs the repo's `check` commands (fail-fast environment validation) and `runBeforeRunHooks` runs `beforeRun` (fail-fast, bring services up). `teardownEnvironment` runs `afterRun` in the dispatcher's `finally` — always, attempt-all (success, block, error, exception). `hooks.ts` executes run-level hooks with `buildBootstrapEnv` (whitelist incl. `DOCKER_*`/`COMPOSE_*`) plus the hook's own token-substituted `env` layered on top (blacklist-bypass, parity with verify env, so allocated ports reach the stack under repo-specific names); `tokens.ts` does substitution; `port.ts` allocates one host-availability-probed port per pool in `environment.ports` (a global async mutex serializes allocation; ports are probed, not promised) and `releaseEnvironmentPorts` frees them on run end so a later run in the same poll pass can reuse them. Per-command service lifecycle (bring a stack up/down for one stateful verify command) lives on each `verify` command's `before`/`after`/`env` and runs in `src/loop/verifier.ts`. There is no shared/dedicated mode.
...
### Metrics (`src/metrics/`)
Prometheus metrics via `prom-client`. `createMetricsService()` returns either a live service or a no-op. All metric calls are wrapped in try-catch — metrics never block or throw. Handoff persistence and handoff-based recovery are counted separately so operators can distinguish normal cross-agent context from crash-recovery fallback. HTTP endpoint serves `/metrics` (Prometheus format) and `/api/stats` (JSON).

### MCP Server (`src/mcp/`)
Model Context Protocol interface for external agents. Twenty-four tools — see [USAGE.md → MCP Integration](./USAGE.md#mcp-integration) for the full list. Three resources (status, config, metrics). Mutation tools require the auth token from `mcp.authTokenEnv` when configured. Transport: `stdio` for the standalone `night-orch mcp` command; HTTP/SSE when embedded in the `run` daemon (host/port from `mcp.httpHost` / `mcp.httpPort`). Run-event streaming exposes one ordered log across system and agent events, and `night-orch-handoffs` exposes the structured cross-agent context for a run.
...
### State (`src/state/`)
SQLite with WAL mode via `better-sqlite3`. `db.ts` handles init and migrations. `attempts.ts` manages the **immutable** attempts ledger (see below): every `retry`/`continue`/`rebase` inserts a new row chained to the previous one via `previous_attempt_id`, and terminated attempts are never mutated afterward. `handoffs.ts` stores the structured handoff documents that move plan/code/verify/review context between agents and can rehydrate missing checkpoint artifacts after a crash. `rebase-fanouts.ts` records per-source-PR fan-out markers plus per-sibling outcomes for idempotent automatic sibling rebases. `runs.ts` preserves the legacy per-row update surface for callers that still query by run id. `leases.ts` provides atomic lease acquisition via `INSERT OR IGNORE`. `stats.ts` powers the Web/TUI dashboards.

### Web (`src/web/`)
HTTP/SSE server serving the Web UI and programmatic API for operators. `server.ts` binds the routes (`/api/status`, `/api/operations/*`, `/api/settings`, `/api/stats`, `/api/cost/health`, `/api/events` SSE). `auth.ts` provides stateless HMAC-signed session cookies (`norch_session`) plus double-submit CSRF cookies for browser mutations; loopback mode stores the generated web token in a `0600` sidecar file instead of exposing it from `/api/session`. The cookie signing secret and the loopback token are persisted in the `web_secrets` table so sessions and the saved token survive restarts. `--skip-auth` remains available for deployments behind a trusted reverse proxy (Caddy, Tailscale serve). `webpush.ts` wires the VAPID-signed Web Push delivery used by the `webpush` notification channel. The frontend is in the top-level `web/` package (Vite + React + Tailwind). Shared DaisyUI component contracts live in `src/components/` and are previewed/documented in Storybook (`pnpm storybook`).
...
- **Post-publish review** — Optional reviewer workers with `runWhen: post-publish` run after PR creation for external tools that need an open PR. These phases are dispatched by finalization, not by the pre-decision loop. Non-approved findings are persisted as external-review handoffs, can be posted to the issue, and can queue a continue pass as `external_review` feedback.
...
If checkpoint `phase_data` is missing or quarantined, `resumeFromCheckpoint()` falls back to persisted handoffs for structured plan, code, verify, and review state. That fallback emits a `recovery_from_handoff` run-log event and increments `night_orch_recovery_from_handoff_total`.
...
| `agent_handoffs` | Loop Engine, Post-publish Review | Loop Engine, CLI, TUI, MCP | Structured cross-agent context (`plan`, `code-summary`, `verify-summary`, `review-findings`, `external-review-findings`) plus markdown previews and token usage. |
...
| `web_secrets` | Web auth (first start) | Web auth (every start) | Persisted loopback mutation token and session-cookie signing secret — keeps sessions and the saved token valid across restarts. |
