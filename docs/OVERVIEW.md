...
One isolation model: a per-run worktree plus **lifecycle hooks**. `manager.ts` allocates per-run substitution tokens (`{issue}`/`{run}`/`{project}` plus one host port per configured pool: `{port}`/`{port:NAME}`) via `prepareEnvironment` (no subprocess), then `runBeforeRunHooks` runs the repo's `beforeRun` commands (fail-fast). `teardownEnvironment` runs `afterRun` in the dispatcher's `finally` — always, attempt-all (success, block, error, exception). `hooks.ts` executes run-level hooks with `buildBootstrapEnv` (whitelist incl. `DOCKER_*`/`COMPOSE_*`); `tokens.ts` does substitution; `port.ts` allocates one free port per pool in `environment.ports` and `releaseEnvironmentPorts` frees them on run end so a later run in the same poll pass can reuse them. Per-command service lifecycle (bring a stack up/down for one stateful verify command) lives on each `verify` command's `before`/`after`/`env` and runs in `src/loop/verifier.ts`. There is no shared/dedicated mode.
